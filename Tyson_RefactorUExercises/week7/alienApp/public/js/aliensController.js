...
aliensFunction.$inject = ['aliensFactory', '$http']

// we pass all of our injected dependencies in as arguments
function aliensFunction(aliensFactory, $http) {
    ...
    aCtrl.submit = function (event) {
        ...
        aCtrl.submitMessage = function(){
           $http.get('/message').then(function success(response){
             console.log('message successful',response.data)
             aCtrl.message = response.data
           },
